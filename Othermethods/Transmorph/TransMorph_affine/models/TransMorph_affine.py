...
class AffineTransformer(nn.Module):
    ...
    def __init__(self, img_size, mode='bilinear'):
        super().__init__()
        D, H, W = img_size
        # Create a 3D grid of coordinates (normalized to [-1, 1])
        ...
        grid = torch.stack([x_coords, y_coords, z_coords, ones], dim=-1).view(-1, 4).T
        self.mode = mode
    ...
    def apply_affine(self, src, mat):
        D, H, W = self.img_size
        transformed_grid = torch.matmul(mat, self.grid)
        transformed_grid = transformed_grid.view(1, 3, D, H, W).permute(0, 2, 3, 4, 1)
        return nnf.grid_sample(src, transformed_grid, align_corners=False, mode=self.mode)
    ...
    def forward(self, src, affine, scale, translate, shear):
        ...
        rot_mat_x = torch.stack([torch.stack([theta_x * 0. + 1, theta_x * 0., theta_x * 0.], dim=1),
                                 torch.stack([theta_x * 0., torch.cos(theta_x), -torch.sin(theta_x)], dim=1),
                                 torch.stack([theta_x * 0., torch.sin(theta_x), torch.cos(theta_x)], dim=1)], dim=2)
        rot_mat_y = torch.stack([torch.stack([torch.cos(theta_y), theta_y * 0., torch.sin(theta_y)], dim=1),
                                 torch.stack([theta_y * 0., theta_y * 0. + 1, theta_y * 0.], dim=1),
                                 torch.stack([-torch.sin(theta_y), theta_y * 0., torch.cos(theta_y)], dim=1)], dim=2)
        rot_mat_z = torch.stack([torch.stack([torch.cos(theta_z), -torch.sin(theta_z), theta_z * 0.], dim=1),
                                 torch.stack([torch.sin(theta_z), torch.cos(theta_z), theta_z * 0.], dim=1),
                                 torch.stack([theta_z * 0., theta_z * 0., theta_z * 0. + 1], dim=1)], dim=2)
        scale_mat = torch.stack(
            [torch.stack([scale_x, theta_z * 0., scale_z * 0.], dim=1),
             torch.stack([scale_x * 0., scale_y, scale_z * 0.], dim=1),
             torch.stack([scale_x * 0., scale_y * 0., scale_z], dim=1)], dim=2)
        shear_mat = torch.stack(
            [torch.stack([theta_x * 0. + 1, torch.tan(shear_xy), torch.tan(shear_xz)], dim=1),
             torch.stack([torch.tan(shear_yx), theta_x * 0. + 1, torch.tan(shear_yz)], dim=1),
             torch.stack([torch.tan(shear_zx), torch.tan(shear_zy), theta_x * 0. + 1], dim=1)], dim=2)
        ...
        transformed_grid = torch.matmul(mat, self.grid)
        transformed_grid = transformed_grid.view(1, 3, D, H, W).permute(0, 2, 3, 4, 1)  # Reshape to NxDxHxWx3
        return nnf.grid_sample(src, transformed_grid, align_corners=False, mode=self.mode), mat, inv_mat
...
class TransMorphAffine(nn.Module):
    ...
    def forward(self, inputs):
        ...
        out = self.transformer(x_cat)  # (B, n_patch, hidden)
