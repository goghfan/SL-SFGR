...
class register_model(nn.Module):
    ...
    def forward(self, x):
        img = x[0].cuda()
        flow = x[1].cuda()
        out = self.spatial_trans(img.unsqueeze(0), flow)
        return out
...
def dice_val_substruct(y_pred, y_true, std_idx):
    with torch.no_grad():
        # 将预测标签转换为one-hot编码，假设有12个类别（包括背景）
        y_pred = nn.functional.one_hot(y_pred, num_classes=12)
        # 去掉张量中维度为1的维度
        y_pred = torch.squeeze(y_pred, 1)
        # 调整张量的维度顺序，以便后续处理
        y_pred = y_pred.permute(0, 4, 1, 2, 3).contiguous()
        
        # 将真实标签转换为one-hot编码，假设有12个类别（包括背景）
        y_true = nn.functional.one_hot(y_true, num_classes=12)
        # 去掉张量中维度为1的维度
        y_true = torch.squeeze(y_true, 1)
        # 调整张量的维度顺序，以便后续处理
        y_true = y_true.permute(0, 4, 1, 2, 3).contiguous()
    
    # 将张量从GPU转移到CPU并转换为numpy数组
    y_pred = y_pred.detach().cpu().numpy()
    y_true = y_true.detach().cpu().numpy()

    # 创建一个包含std_idx的字符串，作为输出行的开头
    line = 'p_{}'.format(std_idx)
    
    # 遍历类别1到11（假设这些是感兴趣的类别）
    for i in range(1, 12):
        # 提取类别i的预测和真实值
