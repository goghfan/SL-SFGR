...
def scan_to_scan(vol_names, bidir=False, batch_size=1, prob_same=0, no_warp=False, **kwargs):
    ...
    while True:
        ...
        invols[0] = (invols[0]-invols[0].min())/(invols[0].max()-invols[0].min())
        invols[1] = (invols[1]-invols[1].min())/(invols[1].max()-invols[1].min())
        outvols[0] = (outvols[0]-outvols[0].min())/(outvols[0].max()-outvols[0].min())
